...
ExplodingTwoWayMultiSprite::ExplodingTwoWayMultiSprite(const TwoWayMultiSprite& s) :
	TwoWayMultiSprite(s),
	chunks(),
	freeList() {
		makeChunks(
...
ExplodingTwoWayMultiSprite::~ExplodingTwoWayMultiSprite() {
	for(Chunk* c : chunks) {
		delete c;
	}
...
void ExplodingTwoWayMultiSprite::draw() const {
	for(Chunk* chunk : chunks) {
		chunk->draw();
	}
}
void ExplodingTwoWayMultiSprite::update(Uint32 ticks) {
...
void ExplodingTwoWayMultiSprite::makeChunks(unsigned int n) {
	...
	while(source_y+chunk_height < getImage()->getHeight()){
		...
		while(source_x + chunk_width < getImage()->getWidth()) {
			...
			float xy = (rand() % speedy + 40) * (rand()%2?-1:1);
