...
/* void IoMod::writeDetails(int fps) const {
   std::ostringstream frameStream;
   frameStream << "fps: " << fps << std::endl;

    IoMod::getInstance().writeText("Jessica Qiu",
				  Viewport::getgdata().getXmlInt("fps/flocX"),
				  Viewport::getgdata().getXmlInt("fps/flocY"),
				  ; 
} */
